...
class Gamo_Reward_Manager {
	...
	*/
	function create_action_type_info($options = array()) {
		...
		*/
		...
		// Ensure that action type info entry with matching information does not already exist
		$c = Core::db_count(array(
				'table' => '' . GAMO_DB . '.action_types_info',
				'values' => array(
					'action_types_id' => $options['action_types_id'],
	...
	*/
	function update_badge($options = array()) {
		...
		*/
		...
		if($options['values'] == 'delete') {
			...
			$sql = 'SELECT count(*) FROM ' . GAMO_DB . '.badges WHERE badge_name = :badge_name AND badge_id != :badge_id';
			$sth = $dbh->prepare($sql);
	...
	/*
	Validate a badge requirbadges_infoement
	*/
	function badges_info_validate($options = array()) {
		...
		arguments:
		{
			...
			badge_id_check: 1 = check that badge is is valid, 0 = do not check if badge id is valid,
			check_duplicate: 1 = test if a duplicate entry exists, 0 = do not check if a duplicate entry exists
		}
		...
		Return:
			...
			{
				validated: {
					badge_id: 1 = badge_id is valid, 0 = badge_id is not valid
					action_types_id: 1 = action_types_id is valid, 0 = action_types_id is not valid,
					info: 1 = information is valid, 0 = information is not valid
				}
			}
		...
		*/
		...
		if(trim($options['info_type']) == '') {
			...
			return Core::error($this->errors, 25);
		...
		*/
		...
		if($options['badge_id_check'] != 0) { // Check badge id
			...
			if($c != 1) { // Badge is not valid
				...
				return Core::error($this->errors, 23);
		...
		if($options['check_duplicate'] != 0) { // Check for a badge info entry with the same information
			...
			if($c > 0) { // A duplicate entry already exists
				...
				return Core::error($this->errors, 26);
	...
	*/
	function create_badge_info($options = array()) {
		...
		*/
		...
		// Determine that the badge id is valid
		$valid = $this->badges_info_validate($options);
	...
	*/
	function get_badges($options = array()) {
		...
		*/
		...
		$sql = 'SELECT badge_id, badge_name, rank FROM ' . GAMO_DB . '.badges ORDER BY badge_name ASC';
