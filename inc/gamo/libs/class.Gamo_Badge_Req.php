	...
	*/
	...
	function req_live_trivia_complete($options = array()) {
		...
		(
			...
			WHERE
			a.quiz_id = " . GAMO_DB . ".quizzes_info.quiz_id
		) AS question_qty
		FROM " . GAMO_DB . ".quizzes_info
		WHERE
		...
		$sql_questions = "SELECT question_id FROM " . GAMO_DB . ".quiz_questions WHERE quiz_id = :quiz_id";
	...
	function req_group_trivia_limit($options = array()) {
		...
		while($row = $sth->fetch()) {
			...
			$c = Core::fetch_column(
				"SELECT SUM(point_value_used) AS total_points FROM " . GAMO_DB . ".actions_log
