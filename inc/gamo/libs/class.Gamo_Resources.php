	...
	*/
	...
	function get_resources_count($options = array()) {
		...
		*/
		...
		$sql = "SELECT count(*)".
		" FROM " . GAMO_DB . ".resources" . $sql_filter;
	...
	*/
		...
		*/
		...
		if($options['stats'] == 1) {
	        ...
	        (
	        	...
	        	AND a.int_other = " . GAMO_DB . ".resources.resource_id
	        ) AS download_qty
			,
	        (
	        	SELECT
		...
		if($options['category'] != '' && $options['category'] != 'all') {
			...
			$category_sql = "(SELECT count(*) FROM " . GAMO_DB . ".resources_info AS a
				WHERE a.resource_id = " . GAMO_DB . ".resources.resource_id AND a.info_type = 'resource_category' AND a.info_a = :resource_category) > 0";
		...
		$sql = "SELECT
		" . GAMO_DB . ".resources.resource_id as resource_id,
		title,
		descrip,
		type,
		location,
		...
		FROM " . GAMO_DB . ".resources" . 
		" LEFT JOIN " . GAMO_DB . ".resources_info as ri on ri.resource_id=" . GAMO_DB . ".resources.resource_id" . 
		$sql_filter . "
	...
	function get_resources_user_count( $options=array() ){
		...
		(
            ...
            WHERE a.resource_id = " . GAMO_DB . ".actions_log.int_other
        ) > 0";
