...
class Gamo_Face_To_Face_Events {
	
	static $table_name = 'face_to_face_events';
	static $action_slug = 'attend_virtual_event'; //todo
	...
	function get_new_events_count(){
		...
		$sql = 	"SELECT count(*) as count FROM ".GAMO_DB.".".self::$table_name.
	...
	function get_new_events( $options=array() ){
		...
		$sql = "SELECT id FROM " . GAMO_DB . "." . self::$table_name.
	...
	*/
	...
	function get_events_user_count( $options=array() ){
		...
		FROM ".GAMO_DB.".".self::$table_name." as ve ".
		"INNER JOIN ".GAMO_DB.".actions_log as al on ve.id=al.int_other ".
	...
	function get_events_user_future( $options=array() ){
		...
		$action_types = Core::r('actions')->action_types_id(array(
				'action_key' => 'rsvp_fevent'
			)
		);
		...
		$sql = 	"SELECT id FROM ".GAMO_DB.".".self::$table_name.
		" WHERE id IN (SELECT int_other FROM `".GAMO_DB."`.`actions_log` WHERE action_types_id = :action_types_id AND ".
	...
	function get_events_user( $options=array() ){
		...
		$sql = 	"SELECT
			...
			(
				SELECT
				int_info
				FROM " . GAMO_DB . ".virtual_events_info AS a
			...
			al.point_value_use+COALESCE( (
				SELECT
					al2.point_value_use
				FROM ".GAMO_DB.".actions_log as al2
				WHERE 
	...
	function get_action_type_for_slug( $options=array() ){
		...
		$sql = "SELECT * FROM `".GAMO_DB."`.`action_types_info` WHERE info_type = :infotype";
