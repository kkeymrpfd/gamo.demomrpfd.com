...
class Gamo_Slots {
    ...
    */
	function get_slots($options = array()) {
        ...
        (
            ...
            WHERE
            a.int_other = " . GAMO_DB . ".slots.slot_id
            AND a.action_types_id = :action_types_id
        ) AS reserved_qty
        FROM " . GAMO_DB . ".slots" . $sql_filter;
    ...
    */
    function unreserve_user_group($options = array()) {
        ...
        $sql = "SELECT slot_id FROM " . GAMO_DB . ".slots WHERE slot_group = :slot_group";
        $sth = $dbh->prepare($sql);
        $sth->execute(array(
        ...
        $sql = "SELECT action_id FROM " . GAMO_DB . ".actions_log WHERE user_id = :user_id AND action_types_id = :action_types_id AND int_other IN (" . implode(",", $slot_ids) . ")";
        $sth = $dbh->prepare($sql);
        $sth->execute(array(
                ':user_id' => $options['user_id'],
                ':action_types_id' => $this->action_type_reserve
        ...
        $delete_sql = "DELETE FROM " . GAMO_DB . ".actions_log WHERE action_id = :action_id";
