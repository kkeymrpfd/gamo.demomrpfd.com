...
class Gamo_Virtual_Events {
	...
	public function add_user( $options=array() ){
		...
		$event_type = Core::fetch_column(
			"SELECT event_type FROM " . GAMO_DB . ".virtual_events WHERE id = :id",
			array(
				':id' => $options['event_id']
			)
		);
		...
		$c = Core::db_count(array(
				'table' => GAMO_DB . '.virtual_events_info',
				'values' => array(
	...
	*/
	...
	function get_events_user_future( $options=array() ){
		...
		$sql = 	"SELECT id FROM ".GAMO_DB.".".self::$table_name.
		" WHERE id IN (SELECT int_other FROM `".GAMO_DB."`.`actions_log` WHERE action_types_id IN (" . $action_types_vevent . ", " . $action_types_ftf . ") AND ".
