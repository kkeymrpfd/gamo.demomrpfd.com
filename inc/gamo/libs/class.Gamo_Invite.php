	...
	*/
	function send_invite($options) {
		...
		*/
		...
		$sql = "UPDATE " . GAMO_DB . ".invites SET sent_qty = sent_qty+1 WHERE invite_id = :invite_id";
		$sth = $dbh->prepare($sql);
		$sth->execute(array(
				':invite_id' => $options['invite_id']
			)
		...
		*/
		...
		if($options['invite_key'] != '') { // Look up the invite_id based on the invite_key
			...
			$id = Core::fetch_column(
					"SELECT invite_id FROM " . GAMO_DB . ".invites WHERE invite_key = :invite_key",
					array(
		...
		*/
		...
		$sql = "SELECT
			...
			(
				...
				AND a.int_other = " . GAMO_DB . ".invites.invite_id
			) AS points,
			(
				SELECT
				end_time
				FROM
				" . GAMO_DB . ".virtual_events AS a
				WHERE
				a.id = " . GAMO_DB . ".invites.event_id
			) AS end_time
			FROM
			" . GAMO_DB . ".invites";
