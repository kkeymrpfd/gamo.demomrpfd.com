...
class Gamo_Survey_Validate {
    ...
    function get_surveys($options = array()){
        ...
        $sql = "SELECT survey_id, title as survey_title, descrip as survey_description FROM " . GAMO_DB . ".`surveys` WHERE user_id = :userid and active = 1 ORDER BY created_time DESC LIMIT 1" .$options['start'] . "," . $options['num'];
    ...
    function validate_survey_validation(&$options = array()){
        ...
        if(isset($options['survey_validate_id'])){
            ...
            if(isset($options['values']['validate_value']) && !isset($options['values']['validate_type'])){
                ...
                $sql = "SELECT survey_validate_type FRMO " . GAMO_DB . ".`survey_validate` WHERE survey_validate_id = :surveyvalidateid and active = 1";
