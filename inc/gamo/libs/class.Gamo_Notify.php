	...
	*/
	function get_notifications($options = array()) {
		...
		*/
		...
		$sql = 'SELECT
			...
			active,
			time
			FROM ' . GAMO_DB . '.notify' . $filter . ' LIMIT ' . (int)$options['start'] . ', ' . (int)$options['records'];
	...
	function notify_seen($options = array()) {
		...
		*/
		...
		$sql = 'UPDATE ' . GAMO_DB . '.notify SET seen = 1 WHERE active = 1 AND user_id = :user_id';
