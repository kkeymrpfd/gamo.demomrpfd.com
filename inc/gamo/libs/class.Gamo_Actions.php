	...
	*/
	function create_action($options = array()) {
		...
		*/
		...
		$sql = "SELECT int_info FROM " . GAMO_DB . ".action_types_info WHERE action_types_id = :action_types_id AND info_type = 'trigger_action'";
		...
		foreach($valid['user']['has'] as $k => $has) {
			...
			if($has['info_type'] == 'link_action_to') { // Link this user to another user or another set of users
				...
				*/
				...
				$sql = "SELECT
				user_id
				FROM " . GAMO_DB . ".users WHERE " . $params['sql'];
	...
	*/
	function map_action_type($action_types_id_alias = -1) {
		...
		*/
		...
		$sql = 'SELECT action_types_id FROM ' . GAMO_DB . '.action_types WHERE action_types_id_alias = :action_types_id_alias';
	...
	*/
	function modify_action($options = array()) {
		...
		*/
		...
		$sql = "SELECT action_id FROM " . GAMO_DB . ".actions_log WHERE triggered_by = :triggered_by AND trigger_type = 2";
		$sth = $dbh->prepare($sql);
		$sth->execute(array(
	...
	*/
	function action_types_id($options = array()) {
		...
		*/
		...
		$sql = "SELECT action_types_id FROM " . GAMO_DB . ".action_types WHERE " . $filters['sql'] . " LIMIT 0, 1";		
	...
	*/
	...
	function get_action_points($options = array()) {
		...
		return Core::fetch_column(
			"SELECT points FROM " . GAMO_DB . ".action_types WHERE action_key = :action_key",
			array(
	...
	function get_action_name($options = array()) {
		...
		return Core::fetch_column(
			"SELECT action_name FROM " . GAMO_DB . ".action_types WHERE action_key = :action_key",
			array(
				':action_key' => $options['action_key']
			)
		);
		...
		*/
		...
		if($options['category_id'] != -1) {
			...
			(SELECT 
				...
				FROM ' . GAMO_DB . '.action_types_info
				WHERE
				' . GAMO_DB . '.action_types_info.action_types_id = ' . GAMO_DB . '.action_types.action_types_id
				AND ' . GAMO_DB . '.action_types_info.info_type = 'category'
				AND ' . GAMO_DB . '.action_types_info.int_info = :category_id) > 0";
		...
		if($options['category_tag'] != -1) {
			...
			" . GAMO_DB . ".action_types_info.action_types_id = " . GAMO_DB . ".action_types.action_types_id
			AND " . GAMO_DB . ".action_types_info.info_type = 'category'
			AND " . GAMO_DB . ".action_types_info.int_info = (SELECT category_id FROM " . GAMO_DB . ".categories WHERE category_tag = :category_tag LIMIT 1) ) > 0";
		...
		$sql = 'SELECT action_types_id, action_name, points FROM ' . GAMO_DB . '.action_types' . $filter;
	...
	function get_actions($options = array()) {
		...
		*/
		...
		$sql = 'SELECT
			...
			active,
			action_id_alias,
			(SELECT display_name FROM ' . GAMO_DB . '.users WHERE ' . GAMO_DB . '.users.user_id = ' . GAMO_DB . '.actions_log.user_id) AS user_display_name,
			(SELECT action_name FROM ' . GAMO_DB . '.action_types WHERE ' . GAMO_DB . '.action_types.action_types_id = ' . GAMO_DB . '.actions_log.action_types_id) AS action_name_display,
			(SELECT action_key FROM ' . GAMO_DB . '.action_types WHERE ' . GAMO_DB . '.action_types.action_types_id = ' . GAMO_DB . '.actions_log.action_types_id) AS action_key
			FROM
			' . GAMO_DB . '.actions_log';
		...
		if(is_numeric($options['category_id']) && $options['category_id'] != -1) { // There is a category filter. Add it
			...
			array_push($filters, "(SELECT
									...
									WHERE
									a.action_types_id = " . GAMO_DB . ".actions_log.action_types_id
		...
		if(trim($options['user_name']) != '') { // There is an action filter. Add it
			...
			array_push($filters, "(SELECT
				...
				FROM " . GAMO_DB . ".users
				WHERE
				" . GAMO_DB . ".users.user_id = " . GAMO_DB . ".actions_log.user_id
					AND (
						" . GAMO_DB . ".users.email LIKE :email
						OR " . GAMO_DB . ".users.first_name LIKE :first_name
						OR " . GAMO_DB . ".users.last_name LIKE :last_name
						OR " . GAMO_DB . ".users.display_name LIKE :display_name
						OR " . GAMO_DB . ".users.company LIKE :company
					)
				) > 0");
		...
		if($options['get_count'] == 1) { // Retrieve how many total records there are
			...
			$sql = 'SELECT
			count(*)
			FROM ' . GAMO_DB . '.actions_log' . $sql_filters;
