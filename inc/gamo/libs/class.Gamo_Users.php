	...
	*/
	function map_user($user_id_alias = -1) {
		...
		*/
		...
		$sql = 'SELECT user_id FROM ' . GAMO_DB . '.users WHERE user_id_alias = :user_id_alias';
		...
		if(!filter_var($options['email'], FILTER_VALIDATE_EMAIL)) {
			...
			*/
			...
			if(is_numeric($options['user_id']) && $options['user_id'] > -1) { // Factor in user_id when testing for uniqueness
				...
				$sql = 'SELECT count(*) FROM ' . GAMO_DB . '.users WHERE email = :email AND user_id != :user_id';
				$params[':user_id'] = $options['user_id'];

			} else { // Do NOT factor in user_id when testing for uniqueness

				$sql = 'SELECT count(*) FROM ' . GAMO_DB . '.users WHERE email = :email';
	...
	*/
	function user_valid($options = array()) {
		...
		*/
		...
		$c = Core::fetch_column(
			'SELECT count(*) FROM ' . GAMO_DB . '.users WHERE user_id = :user_id',
			array(
				':user_id' => $options['user_id']
			)
		);
	...
	function get_user($options = array()) {
		...
		*/
		...
		if($rank_k !== FALSE) { // the rank field was found in the fields list. Update it with the actual sub-query value
			...
			$options['fields'][$rank_k] = '(SELECT count(DISTINCT points) FROM ' . GAMO_DB . '.users AS a WHERE ' . GAMO_DB . '.users.points < a.points AND ' . GAMO_DB . '.users.user_group = a.user_group' . $filter_sql;

			if($options['rank_type'] != 'all') {

				$options['fields'][$rank_k] .= ' AND ' . GAMO_DB . '.users.other = a.other';
		...
		if($options['get_has'] == 1) {
			...
			badge_id,
			rank,
			(SELECT badge_name FROM ' . GAMO_DB . '.badges WHERE ' . GAMO_DB . '.badges.badge_id = ' . GAMO_DB . '.users_info.badge_id LIMIT 0, 1) AS badge_name,
			info_type,
			int_info,
			info,
			info_b,
			...
			if($has_rank) {
				...
				$user_info['level_min_points'] = Core::fetch_column(
					"SELECT points FROM " . GAMO_DB . ".badges_info WHERE badge_id = :badge_id AND info_type = 'min_points'",
					array(
				...
				$user_info['level'] = Core::fetch_column(
					"SELECT badge_name FROM " . GAMO_DB . ".badges WHERE rank > 0 AND active = 1 ORDER BY rank ASC LIMIT 0, 1",
					array()		
				);
	...
	*/
	function get_users($options = array()) {
		...
		*/
			...
			if(isset($options['filters']['badges']) && count($options['filters']['badges']) > 0) { // Filter by badge(s)
				...
				foreach($options['filters']['badges'] as $k => $badge_id) {
					...
					array_push($filters, "(SELECT
						...
						FROM " . GAMO_DB . ".users_info
						WHERE
						" . GAMO_DB . ".users_info.user_id = " . GAMO_DB . ".users.user_id
						AND " . GAMO_DB . ".users_info.badge_id = :badge_id" . $k . "
						) > 0
					");
			...
			if(isset($options['filters']['action_types']) && count($options['filters']['action_types']) > 0) { // Filter by action(s)
				...
				foreach($options['filters']['action_types'] as $k => $action_type) {
					...
					array_push($filters, "(SELECT
						...
						WHERE
						" . GAMO_DB . ".actions_log.user_id = " . GAMO_DB . ".users.user_id
						AND " . GAMO_DB . ".actions_log.action_types_id = :action_types_id" . $k . "
						) >= :action_types_qty" . $k . "
					");
		...
		$sql = 'SELECT user_id FROM ' . GAMO_DB . '.users' . $sql_filter . ' ORDER BY ' . $options['sort'] . ' LIMIT ' . (int)$options['start'] . ', ' . (int)$options['records'];
		...
		$sql = 'SELECT count(*) FROM ' . GAMO_DB . '.users' . $sql_filter;
		$sth = $dbh->prepare($sql);
		$sth->execute($params);
