        ...
        */
        ...
        foreach($this->action_types_ids as $k => $v) {
            ...
            $id = Core::fetch_column(
                "SELECT action_id FROM " . CORE_DB . ".actions_log WHERE action_types_id = :action_types_id AND user_id = :user_id AND other = :other AND active = 1",
                array(
                    ':action_types_id' => $v,
                    ':user_id' => $options['user_id'],
                    ':other' => $options['email']
    ...
    function create_meeting($options = array()) {
        ...
        if($options['status_exists']['submit_meeting'] === FALSE) {        
            ...
            $hash = Core::fetch_column(
                "SELECT other_b FROM " . CORE_DB . ".actions_log WHERE action_id = :action_id",
                array(
        ...
        while($row = $sth->fetch()) {
            ...
            if($k != -1) {
                ...
                $record['company'] = Core::fetch_column(
                    "SELECT info FROM " . CORE_DB . ".actions_info WHERE action_id = :action_id AND info_type = 'company' LIMIT 0, 1",
                    array(
                        ':action_id' => $row['action_id']
                    )
                );
                ...
                $record['total_points'] = Core::fetch_column(
                    "SELECT SUM(point_value_used) FROM " . CORE_DB . ".actions_log WHERE action_types_id IN (" . implode(',', $this->action_types_ids) . ") AND user_id = :user_id AND other = :other AND active = 1",
                    array(
                        ':other' => $record['other'],
                        ':user_id' => $options['user_id']
                    )
