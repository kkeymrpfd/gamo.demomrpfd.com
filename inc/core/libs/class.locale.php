<?
/*
This class handles the localization for ' . CORE_DB . '.
*/
class Core_Locale {
	...
	function __construct() {
		...
		$this->r('locale')_id = $this->english;
	...
	*/
	...
	mapping the user to the locale ID in ' . CORE_DB . '.
	*/
	function map_locale($locale_id_alias = '') {
		...
		*/
		...
		$sql = 'SELECT locale_id FROM ' . CORE_DB . '.locales WHERE locale_id_alias = :locale_id_alias';
	...
	function update_locale($options = array()) {
		...
		*/
		...
		if(is_array($options['values'])) { // Update record
			...
			$sql = 'DELETE FROM ' . CORE_DB . '.locales WHERE locale_id = :locale_id';
	...
	function get_locales() {
		...
		*/
		...
		$sql = 'SELECT
			...
			locale_name,
			locale_id_alias
			FROM ' . CORE_DB . '.locales ORDER BY locale_name ASC';
	...
	function update_text($options = array()) {
		...
		*/
		...
		if(!is_numeric($options['locale_text_id'])) { // The entry name was given instead of the id. Retrieve the id
			...
			$sql = 'SELECT locale_id FROM ' . CORE_DB . '.locale_text WHERE text_name = :text_name';
		...
		if(isset($options['values']['text_name'])) { // The text name is being updated. Test it to ensure validity
			...
			if(isset($options['values']['locale_id'])) { // The locale id is specified for updating. Use that
				...
				$sql = 'SELECT locale_id FROM ' . CORE_DB . '.locale_text WHERE locale_text_id = :locale_text_id';
				$sth = $dbh->prepare($sql);
				$sth->execute(array(
			...
			$sql = 'SELECT count(*) FROM ' . CORE_DB . '.locale_text WHERE locale_text_id != :locale_text_id AND text_name = :text_name AND locale_id = :locale_id';
			$sth = $dbh->prepare($sql);
			$sth->execute(array(
					':locale_text_id' => $options['locale_text_id'],
	...
	function get($text_name, $locale_id = 'get') {
		...
		*/
		...
		if($locale_id == 'get') {
			...
			global $session;
			$locale_id = $this->r('locale')_id;

		}

		$text = Core::fetch_column(
			'SELECT info FROM ' . CORE_DB . '.locale_text WHERE text_name = :text_name AND locale_id = :locale_id',
			array(
				':text_name' => $text_name,
				':locale_id' => $locale_id
			)
