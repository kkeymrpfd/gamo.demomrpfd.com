...
class Core_Rewards {
	...
	function __construct() {
		...
		$this->errors = array(
			...
			array(
				'error_code' => '8',
				'error_msg' => 'Tried evaluating an undefined badge requirement'
			),
			array(
				'error_code' => '9',
	...
	*/
	function calc_rewards($options) {
		...
		*/
		...
		$sql = 'UPDATE ' . CORE_DB . '.actions_log SET point_value_used = :point_value_use WHERE action_id = :action_id';
		$sth = $dbh->prepare($sql);
		$sth->execute(array(
		...
		if($action['point_value_used'] != 0) { // There were already points calculated for this action. Undo that
			...
			$sql = 'UPDATE ' . CORE_DB . '.users SET points = points - :point_value_used WHERE user_id = :user_id';
			$sth = $dbh->prepare($sql);
			$sth->execute(array(
		...
		$sql = 'UPDATE ' . CORE_DB . '.users SET points = points + :point_value_use WHERE user_id = :user_id';
		$sth = $dbh->prepare($sql);
		$sth->execute(array(
				':point_value_use' => $action['point_value_use'],
				':user_id' => $action['user_id']
	...
	*/
	function full_calc_rewards($options = array()) {
		...
		*/
		...
		// update the actions_log
		$sql = 'UPDATE ' . CORE_DB . '.actions_log SET point_value_used = point_value_use WHERE user_id = :user_id';
		$sth = $dbh->prepare($sql);
		$sth->execute(array(
				':user_id' => $options['user_id']
			)
		...
		$sql = 'UPDATE ' . CORE_DB . '.users SET points = :total_points WHERE user_id = :user_id';
		$sth = $dbh->prepare($sql);
		$sth->execute(array(
	...
	*/
	function determine_badges($options = array()) {
		...
		Iterate through each badge_id, and determine the following:
			...
			2) The user has met all badge requirements. This is done by iterating through each badge requirement via this->r('badge_reqs')()
		...
		*/
		...
		if($options['badge_state'] == 'new') { // Only check badges that this user does not already have
			...
			$filter = ' AND (SELECT count(*) FROM ' . CORE_DB . '.users_info WHERE ' . CORE_DB . '.users_info.user_id = :user_id AND ' . CORE_DB . '.users_info.badge_id = ' . CORE_DB . '.badges.badge_id) = 0';
		...
		badge_id
		FROM ' . CORE_DB . '.badges
		WHERE (SELECT count(*) FROM ' . CORE_DB . '.badges_info WHERE ' . CORE_DB . '.badges_info.badge_id = ' . CORE_DB . '.badges.badge_id AND ' . CORE_DB . '.badges_info.is_req = 1) > 0' . $filter;
	...
	*/
	function revoke_badge($options = array()) {
		...
		*/
		...
		$sql = 'SELECT int_info FROM ' . CORE_DB . '.users_info WHERE user_id = :user_id AND badge_id = :badge_id';
		$sth = $dbh->prepare($sql);
		$sth->execute(array(
				':user_id' => $options['user_id'],
		...
		$sql = 'DELETE FROM ' . CORE_DB . '.users_info WHERE user_id = :user_id AND badge_id = :badge_id';
		$sth = $dbh->prepare($sql);
		$sth->execute(array(
				':user_id' => $options['user_id'],
				':badge_id' => $options['badge_id']
	...
	To test if a user has met all the requirements for a badge, use $this->r('badge_reqs')()
	*/
	function give_badge($options = array()) {
		...
		*/
		...
		$sql = 'SELECT badge_name, rank, int_info, info, info_b, trigger_action FROM ' . CORE_DB . '.badges WHERE badge_id = :badge_id';
		$sth = $dbh->prepare($sql);
		$sth->execute(array(
				':badge_id' => $options['badge_id']
			)
		...
		$insert = Core::r('users')->create_user_info(array(
				'user_id' => $options['user_id'],
				'badge_id' => $options['badge_id'],
				'rank' => $badge['rank'],
		...
		if($badge['trigger_action'] != -1) { // This badge is worth points. Factor it into the user's points
			...
			/*
			$other['create_action'] = Core::r('actions')->create_action(array(
					'user_id' => $options['user_id'],
...
<a href="http://twitter.com/intent/tweet?url=www.trivialegacy.com&text=I earned the ' . $badge['badge_name'] . ' level in the Momentum Trivia Legacy game at EMC Conference www.trivialegacy.com&related=yarrcat" target="_blank">Share on Twitter</a>
		...
		if($badge['rank'] > 0) { // Do not create a notification for the lowest rank since everyone already has that
			...
			if($options['badge_id'] != -1) {


				$params['badge_id'] = $options['badge_id'];

			}

			// Notify the user
			Core::r('notify')->create_notify($params);
	...
	function check_badge_req($options = array()) {
		...
		*/
		...
		if(!is_array($options['badges_info'])) {
			...
			$options['badges_info'] = Core::r('reward_manager')->get_badges_info(array(
		...
		if(!method_exists(Core::r('badge_req'), $method)) { // This is not a valid requirement type

			return Core::error($this->errors, 8);

		}
		
		$result = Core::r('badge_req')->$method(array(
				'user' => $options['user'],
