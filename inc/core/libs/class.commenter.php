...
class Core_Commenter{
	...
	(
		...
		AND a.int_other = " . CORE_DB . ".comments.comment_id
		AND active = 1
		AND point_value_use > 0
	) AS points
            FROM " . CORE_DB . ".comments
	...
	$sql = "SELECT
		...
		(
			SELECT
			display_name
			FROM " . CORE_DB . ".users AS a
			WHERE a.user_id = " . CORE_DB . ".comments.user_id
		) AS display_name
                FROM " . CORE_DB . ".comments
