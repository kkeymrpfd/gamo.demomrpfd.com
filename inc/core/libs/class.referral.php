...
class Core_Referral {
	...
	*/
	...
	if($c > 0) {
        ...
        if($c == 0) { // This is a new referral
	    	...
	    	$referral_id = Core::fetch_column(
	    		"SELECT referral_id FROM " . CORE_DB . ".referrals WHERE email = :email AND user_claimed <= 0",
	    		array(
	    			':email' => $options['email']
	    		)
	    	);
	...
	function send_referral($options = array()) {
		...
		if($options['referral_id'] != -1) {
			...
			user_claimed,
			msg
			FROM " . CORE_DB . ".referrals WHERE referral_id = :referral_id";
		...
		$sql = "UPDATE " . CORE_DB . ".referrals SET sent_qty = sent_qty+1 WHERE referral_id = :referral_id";
		$sth = $dbh->prepare($sql);
		$sth->execute(array(
				':referral_id' => $options['referral_id']
			)
	...
	*/
		...
		*/
        ...
        elseif(!isset($options['referral_id'])){
            ...
            $emailExists = Core::fetch_column(
            	"SELECT count(*) FROM " . CORE_DB . ".referrals WHERE email = :email AND user_claimed > 0",
            	array(
            		':email' => $options['email']
            	)
            );
            ...
            $sql = 'SELECT count(*) FROM ' . CORE_DB . '.referrals WHERE user_created = :user_created AND created_datetime BETWEEN :start_date AND :end_date';
	...
	function get_nominations_count(){
		...
		$sql = "SELECT count(*) as count FROM ".CORE_DB.".`referrals` WHERE user_claimed = -1 AND active = 1";
	...
	function get_referrals( $options=array() ){
		...
		(
			...
			AND a.int_other = ".CORE_DB.".referrals.referral_id
			AND a.point_value_use > 0
			LIMIT 0, 1
		) AS points
		FROM ".CORE_DB.".referrals
	...
	function get_nominations( $options=array() ){
		...
		FROM ".CORE_DB.'.referrals WHERE user_claimed = -1 AND active = 1 ORDER BY referral_id ASC LIMIT '.$options['start'].','.$options['num'];
