	...
	*/
	function user_stats() {
		...
		*/
		...
		count(*),
		SUM(points) as total_points
		FROM ' . CORE_DB . '.users WHERE points > 0';
		...
		*/
		...
		$sql = 'SELECT
			...
			(
				...
				WHERE ' . CORE_DB . '.actions_log.action_types_id = ' . CORE_DB . '.action_types.action_types_id AND ' . CORE_DB . '.actions_log.active = 1' . $filter_params['sql'] . '
			) AS action_qty
