		...
		*/
		...
		if(isset($data['user_id']) && is_numeric($data['user_id']) && $data['user_id'] > 0) {
			...
			$start_time = Core::fetch_column(
				"SELECT start_time FROM " . CORE_DB . ".quiz_answer_times WHERE user_id = :user_id AND question_id = :question_id",
				array(
		...
		*/
		...
		if($options['quiz_id'] == '') {
			...
			$options['quiz_id'] = Core::fetch_column(
				"SELECT quiz_id FROM " . CORE_DB . ".quizzes WHERE quiz_name = :quiz_name",
				array(
	...
	*/
	function start_question_timer($options = array()) {
		...
		$time_id = Core::fetch_column(
			"SELECT time_id FROM " . CORE_DB . ".quiz_answer_times WHERE question_id = :question_id AND user_id = :user_id",
			array(
				':question_id' => $options['question_id'],
		...
		$time_id = Core::db_insert(array(
				'table' => CORE_DB . '.quiz_answer_times',
				'values' => array(
					'question_id' => $options['question_id'],
					'user_id' => $options['user_id'],
	...
	function answer_question($options) {
		...
		*/
		...
		if($options['poll_answers'] == '') {
			...
			(
				...
				WHERE
				a.quiz_id = " . CORE_DB . ".quiz_questions.quiz_id
				AND a.sort_order > " . CORE_DB . ".quiz_questions.sort_order
			...
			(
				SELECT
				count(*)
				FROM " . CORE_DB . ".quiz_answers AS a
				WHERE a.question_id = " . CORE_DB . ".quiz_questions.question_id
		...
		*/
		...
		$answer_correct_text = Core::fetch_column(
			"SELECT answer FROM " . CORE_DB . ".quiz_answers WHERE question_id = :question_id AND correct = 1",
			array(
				':question_id' => $options['question_id']
			)
		);
		...
		$answer_given_text = Core::fetch_column(
			"SELECT answer FROM " . CORE_DB . ".quiz_answers WHERE question_id = :question_id AND answer_id = :answer_id",
			array(
				':question_id' => $options['question_id'],
				':answer_id' => $options['answer_id']
			)
	...
	function next_question($options) {
		...
		*/
		...
		$user_group = Core::fetch_column(
			"SELECT info FROM " . CORE_DB . ".users_info WHERE info_type = 'user_group' AND user_id = :user_id LIMIT 0, 1",
			array(
				':user_id' => $options['user_id']
			)
		);
	...
	*/
	function user_next_question($options = array()) {
		...
		*/
		...
		$sql = "SELECT question_id FROM " . CORE_DB . ".quiz_questions WHERE quiz_id = :quiz_id ORDER BY sort_order ASC";
		$sth = $dbh->prepare($sql);
		$sth->execute(array(
				':quiz_id' => $options['quiz_id']
			)
		...
		$required_qr_id = Core::fetch_column(
			"SELECT int_info FROM " . CORE_DB . ".quizzes_info WHERE quiz_id = :quiz_id AND info_type = 'qr_required'",
			array(
				':quiz_id' => $options['quiz_id']
			)
		);
		...
		$required_action = Core::fetch_column(
			"SELECT int_info FROM " . CORE_DB . ".quizzes_info WHERE quiz_id = :quiz_id AND info_type = 'action_required'",
			array(
				':quiz_id' => $options['quiz_id']
			)
		);
