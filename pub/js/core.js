...
var Core = new function() {
	...
	this.adjustHeights = function (elem) {
	...
	this.compare_to_local_offset = function(raw, offset){
		
		var tmp_date = new Date();
		var utc_offset = tmp_date.getTimezoneOffset()*-1;
		
		return moment(raw).isAfter( moment().subtract('h',offset) );
...
$(document).ready(function() {
	...
	$(':checkbox').on('change', null, function() {
          ...
          $("*[check-bind='"+$(this).attr('id')+"']").filter(function() {
               ...
               fontWeight: 'normal' //($(this).prop('checked')) ? 'bold' : 'normal'
          });
